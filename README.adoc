...
:LD_PRELOAD: pass:attributes,quotes[*LD_PRELOAD* ({1}[*ld.so*(8)]), or *DYLD_INSERT_LIBRARIES* on macOS]
...
ifdef::manmanual[]
:LD_PRELOAD: pass:quotes[*LD_PRELOAD* (see *ld.so*(8)), or *DYLD_INSERT_LIBRARIES* on macOS]
